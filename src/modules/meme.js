...
const memeCommand = new SlashCommandBuilder().setDescription('replies with a random meme');

memeCommand.addSubcommand(subCommand => subCommand.setName('random').setDescription('replies with a random meme'));
memeCommand.addSubcommand(subCommand => subCommand.setName('subreddits').setDescription('shows a list of available subreddits'));
memeCommand.addSubcommand(subCommand => subCommand.setName('add')
  .setDescription('adds a subreddit to the list of subreddits')
  .addStringOption(option => option.setName('subreddit').setDescription('the subreddit to add')));
memeCommand.addSubcommand(subCommand => subCommand.setName('remove')
...
const memeCommandHandler = async (interaction) => {
  ...
  switch (subCommand) {
    ...
    case 'random': {
      ...
      interaction.reply(`This meme is from r/${meme.subreddit}\n ${meme.url}`);
    }
  };
};
